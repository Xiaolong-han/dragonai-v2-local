    ...
    if chat_request.stream:
        async def generate():
            ...
            try:
                ...
                logger.info(f"[SSE] Client disconnected, conversation_id={chat_request.conversation_id}")
                if full_response:
                    metadata = {"model": "expert" if chat_request.is_expert else "fast", "incomplete": True}
                    if thinking_content:
                        metadata["thinking_content"] = thinking_content
                    await chat_service.create_message(
                        db,
                    ...
                    logger.info(f"[SSE] Saved incomplete response, length: {len(full_response)}")
                raise
            except asyncio.CancelledError:
                logger.info(f"[SSE] Request cancelled, conversation_id={chat_request.conversation_id}")
                if full_response:
                    metadata = {"model": "expert" if chat_request.is_expert else "fast", "incomplete": True}
                    if thinking_content:
                        metadata["thinking_content"] = thinking_content
                    ...
                    logger.info(f"[SSE] Saved incomplete response on cancel, length: {len(full_response)}")
                raise
